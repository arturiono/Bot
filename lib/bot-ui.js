...
class BotUI {
    constructor(token, opt, events) {
        ...
        events.forEach((event) => {
            this.bot.on(event, (obj) => __awaiter(this, void 0, void 0, function* () {
                let callbackChatId = '';
                if (event === 'message')
                ...
                if (this.replyContext[callbackChatId] &&
                    ...
                    try {
                        ...
                        yield this.replyContext[callbackChatId][event](obj);
                    }
                    catch (e) {
                        yield this.catch(this.replyContextMsg[callbackChatId], e);
                    }
                }
                return;
            }));
    ...
    // сохраняет все оишбки в trycatch.log'
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            fs.writeFile('trycatch.log', `Time: ${new Date().toString()}\n` +
                `Caught exception: ${e}\n`, { flag: 'a+' }, (err) => { });
            yield this.message(msg, '*❗️ Произошла неизвестная ошибка!* Попробуй заново воспользоватся меню');
        });
    }
    commands(obj) {
